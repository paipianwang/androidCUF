...
public abstract class CubeFragmentActivity extends FragmentActivity {
    ...
    private void goToThisFragment(FragmentParam param) {
        ...
        try {
            ...
            if (DEBUG) {
                LogUtil.d( String.format("before operate, stack entry count: %s", fm.getBackStackEntryCount()));
            }
            ...
            if (fragment.isAdded()) {
                if (DEBUG) {
                    LogUtil.d( String.format("%s has been added, will be shown again.", fragmentTag));
                }
                ft.show(fragment);
            } else {
                if (DEBUG) {
                	LogUtil.dd("%s is added.", fragmentTag);
                }
